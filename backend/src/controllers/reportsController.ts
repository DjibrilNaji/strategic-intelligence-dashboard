...
export async function getGlobalReport(c: Context) {
  ...
  const averageWeatherScore =
    bestCountries.reduce((sum, r) => sum + Number(r.weatherScore), 0) /
    bestCountries.length;
...
export async function getDeepAnalysis(c: Context) {
  ...
  const globalAverageScore =
    allCountryReports.reduce((sum, r) => sum + Number(r.score), 0) /
    allCountryReports.length;

  const globalAverageWeatherScore =
    allCountryReports.reduce((sum, r) => sum + Number(r.weatherScore), 0) /
    allCountryReports.length;
